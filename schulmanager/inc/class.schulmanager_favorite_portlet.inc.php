 ...
 */
class schulmanager_favorite_portlet extends home_favorite_portlet
{
	/**
	...
	/**
	 * Override from notenmanager to clear the app header
	 *
	 * @param type $query
	 * @param type $rows
	...
	public static function process($content = array())
	{
		...
		if ($content['nm']['action'])
		{
			...
			else
			{
				...
				elseif(empty($msg))
				{
					$msg .= lang('%1 notenmanager(s) %2, %3 failed because of insufficent rights !!!',$success,$action_msg,$failed);
				}
				Api\Json\Response::get()->apply('egw.message',array($msg,'error'));
			}
		}
