...
class schulmanager_export_kv_pdf extends schulmanager_export_pdf //FPDF
{
	...
	function __construct($rows, $klassenname)
	{
		//$this->appendFachGSk($rows, $klassenname);
	...
	function createPDFKlassenNotenListe(){
		...
		foreach($this->rows as $row)
		{
			...
			// Block Mitzteilung absinkende Leistung
		...
		$date = new DateTime();
		$filename = str_replace(' ', '_', $this->title).'_Notenbogen_'.$date->format('Y-m-d').'.pdf';

		$strPDF = $this->Output('D', $filename);
		return $strPDF;

		//$strPDF = $this->Output('S', $filename);
		//$test = substr($strPDF, 0, 20);
		//return $test;
	...
	function createInfoSchueler($row){
		...
		//$this->Cell(120,8,$this->klassenname,0,0,'L');
		$this->Cell(120,8,$this->title.' '.$name,0,0,'L');
		// Title
		$this->Cell(30,8,'Notenbogen',0,0,'C');
	...
	function createTable($header, $faecher){
		...
		foreach($faecher as $fach)
		{
			...
			$schnitt_hj_1 = $fach['noten']['schnitt_hj_1']['-1']['note'];
			$schnitt_hj_1_manuell = $fach['noten']['schnitt_hj_1']['-1']['manuell'];

			$note_hj_1 = $fach['noten']['note_hj_1']['-1']['note'];
			$note_hj_1_manuell = $fach['noten']['note_hj_1']['-1']['manuell'];

			$m_hj_1 = $fach['noten']['m_hj_1']['-1']['note'];
			$v_hj_1 = $fach['noten']['v_hj_1']['-1']['note'];
			// 2. Halbjahr
			$glnw_hj_2 = $fach['noten']['glnw_hj_2']['concat'];
			$klnw_hj_2 = $fach['noten']['klnw_hj_2']['concat'];

			$glnw_hj_2_avg =		 $fach['noten']['glnw_hj_2']['-1']['note'];
			$glnw_hj_2_avg_manuell = $fach['noten']['glnw_hj_2']['-1']['manuell'];

			$klnw_hj_2_avg = $fach['noten']['klnw_hj_2']['-1']['note'];
			$klnw_hj_2_avg_manuell = $fach['noten']['klnw_hj_2']['-1']['manuell'];

			$schnitt_hj_2 = $fach['noten']['schnitt_hj_2']['-1']['note'];
			$schnitt_hj_2_manuell = $fach['noten']['schnitt_hj_2']['-1']['manuell'];

			$note_hj_2 = $fach['noten']['note_hj_2']['-1']['note'];
			$note_hj_2_manuell = $fach['noten']['note_hj_2']['-1']['manuell'];

			$m_hj_2 = $fach['noten']['m_hj_2']['-1']['note'];
			$v_hj_2 = $fach['noten']['v_hj_2']['-1']['note'];
