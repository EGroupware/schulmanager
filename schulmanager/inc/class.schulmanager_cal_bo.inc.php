...
/**
 * Business object of the Schulmanager
 *
...
class schulmanager_cal_bo extends Api\Storage
{
	...
	{
		//parent::__construct(SCHULMANAGER_APP,'egw_schulmanager',self::EXTRA_TABLE,'','ts_extra_name','ts_extra_value','ts_id');
	...
	function searchCalDates(&$query_in, &$rows){
		...
		foreach($items as $key => $calItem){
			...
			if($rowIndex >= 0){
				...
				//$rows[$rowIndex]['cal'] = $this->getCalViewData($calItem, $start, $end);
	...
	function searchCalDatesInRange($start, $end, $klassenname, $klassengruppe){
		...
		foreach($items as $key => $calItem){
			...
			/*if($rowIndex >= 0){
				$this->getCalViewData($calItem, $start, $end, $rows[$rowIndex]['cal']);rg
			}*/
		}
		return $result;
	}
	...
	function isReadonly($event){
		...
		// if user set as coordinator
		//if(in_array($GLOBALS['egw_info']['user']['account_id'], $config['cal_coordinator'])){
		//	$readonly = false;
		//}
		if($this->activeUserIsCalCoordinator()){
	...
	/**
	 * deletes event if timestamp has not been updates allowed
	 * @param type $id
	 */
	...
	function writeEvent($content, $id, $date, $klasse, $klassengruppe, &$msg){
		...
		$event = array(
			...
			'end' => $end,
			'owner' => $owner,
			//'description' => 'Klasse: '.$klasse.'('.$klassengruppe.'; Fach: '.$fachName.'; Typ:'.$typeName.'; user: '.$userName,
		...
		//if(!$this->checkEventModified($content, $id, $date, $klasse, $klassengruppe, $modified, $msg)){
	 ...
	 * @param type $msg
	 */
	//function checkEventModified($content, $id, $date, $klasse, $klassengruppe, $modified, &$msg){
	...
	public function getDateByColID($colID){
		...
		//$tag = $dateSelected->format('w');
		//return $this->tage[$tag].$dateSelected->format(' d.m.Y');
		return $dateSelected;
	...
	function modifyBeforeSend2Client(array &$itemDesc, $itemSrc){
		...
		//if($userID == $calItem[SCHULMANAGER_CAL_USER]){
	...
	function saveAccessConfig($content){
		...
		if ($content['button']['save'] || $content['button']['apply'])
		{
			...
			Api\Config::save_value('sm_cal_access_accounts', $accessAccounts, $appname);
			Api\Config::save_value('sm_cal_access_groups', $accessGroups, $appname);


			if(!$errors && !$content['apply'])
			{
	...
	function getCalKlassenGruppen(&$query_in,&$rows,$showAllGroups = 0)
	{
		...
		$this->so_lehrer->getKlassenGruppen($username, $klassen, $showAllGroups, $query_in['search']);
